...
if (($count_elements_page !== false) && in_array($type, array('hash', 'list', 'set', 'zset')) && ($size > $count_elements_page)) {
    ...
    if ($page_num_request > 1) {
        $pagination .= "<a href=\"$url&page=$prev\">&#8592;</a>&nbsp;";
    } else {
        $pagination .= "&#8592;&nbsp;";
    }
    ...
    if ($lastpage < 7 + ($adjacents * 2)) { //not enough pages to bother breaking it up
        for ($counter = 1; $counter <= $lastpage; $counter++) {
            if ($counter == $page_num_request) {
                $pagination .= $page_num_request . '&nbsp;';
            } else {
    ...
    if ($page_num_request < $counter - 1) {
        $pagination .= "<a href=\"$url&page=$next\">&#8594;</a>&nbsp;";
    } else {
        $pagination .= "&#8594;&nbsp;";
    }
    $pagination .= "</div>";
...
<table>
<tr> 
<td><a target="_blank" href="json_view.php?<?php echo  http_build_query($_GET);?>" > JSONView</a></td> 
...
<?php foreach ($values as $hkey => $value) { ?>
  ...
  <td><a target="_blank" href="json_view.php?<?php echo  http_build_query($_GET+['hkey'=>$hkey]);?>" > JSONView</a></td> 

  <td><div><?php echo format_html($hkey)?></div></td><td><div class=data><?php echo format_html($value)?></div></td><td><div>
